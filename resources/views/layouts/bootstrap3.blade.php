	                    ...
	                    <select class="selectpicker tasks-bar id_{{$titleBreadCumbs[0]}}" data-live-search="true" id="dropdownBreadcrumbs">
	                    	@if (count($titleBreadCumbs[1]) > 0)
		                    		@foreach ($titleBreadCumbs[1] as $key => $account)
	                                    <option data-breadcumbs="{{$key}}" data-tokens="{{$account}}" {{ (int)$key === (int)$titleBreadCumbs['flag'] ? "selected" : ""}} data-url= "{{ $breadcrumb->url }}" >
	                                    <a href="#">
				...
				<li class="breadcrumb-item active">
					<div class="breadcrumb-item-detail">
	                    ...
	                    <select class="selectpicker tasks-bar id_{{$titleBreadCumbs[0]}}" data-live-search="true" id="dropdownBreadcrumbs">
	                    	@if (count($titleBreadCumbs[1]) > 0)
	                    		@foreach ($titleBreadCumbs[1] as $key => $account)
                                    <option data-breadcumbs="{{$key}}" data-tokens="{{$account}}" data-url= "{{ $breadcrumb->url }}" {{ (int)$key === (int)$titleBreadCumbs['flag'] ? "selected" : ""}} >
                                    <a href="#">
                                        <div class="desc" >{{$account}}</div>
                                    </a>
                                </option>

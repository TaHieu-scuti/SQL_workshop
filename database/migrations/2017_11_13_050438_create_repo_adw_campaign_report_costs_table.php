        ...
        Schema::create(
            ...
            function (Blueprint $table) {
                ...
                $table->double('activeViewViewableImprMeasurableImpr')
                    ->nullable()
                    ->comment(
                        '広告がアクティブビュー対応サイトに表示された時間（測定可能なインプレッション数）
                ...
                $table->double('mobileBidAdj')
                    ->nullable()
                    ->comment(
                        'キャンペーンのモバイル入札単価調整機能。このフィールドでフィルタリングするには、
                ...
                $table->double('ctr')
                    ->nullable()
                    ->comment(
                        '広告がクリックされた回数（クリック数）を広告が表示された回数

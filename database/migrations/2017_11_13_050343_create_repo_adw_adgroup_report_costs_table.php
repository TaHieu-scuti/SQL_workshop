    ...
    public function up()
    {
        Schema::create(
            ...
            function (Blueprint $table) {
                ...
                $table->string('contentNetworkBidDimension', 50)
                    ->nullable()
                    ->comment('広告グループでディスプレイネットワークの絶対的な入札単価に使用する条件のタイプ。');
                ...
                $table->double('conversions')
                    ...
                    ->comment(
                        ...
                        . '小数点の区切り文字として小数点以下2桁の小数点以下桁区切り記号（例：1000000.00）のドット（"."）でフォーマットされます。'
                    );
                $table->double('totalConvValue')
                    ->nullable()
                    ->comment(
                        'すべてのコンバージョンのコンバージョン値の合計。このフィールドは、小数点の区切り文字として小数点以下2桁の小数点以下桁区切り記号'
                        . '（例：1000000.00）のドット（"."）でフォーマットされます。'
                    );
                ...
                $table->double('costConvCurrentModel')
                    ->nullable()
                    ->comment(
                        '現在選択しているアトリビューションモデルで、過去の「CostPerConversion」データがどのように表示されるかを示します。このフィールドは、'
                        . '小数点の区切り文字として小数点以下2桁の小数点以下桁区切り記号（例：1000000.00）のドット（"."）でフォーマットされます。'
                    );
                $table->double('defaultMaxCPC')
                    ->nullable()
                    ->comment(
                        'クリック単価制。値は、a）小額の金額、b）AdWordsが自動的に選択された入札戦略で入札単価を設定する場合は「自動：x」または「自動」、'
                ...
                $table->double('maxCPV')
                    ->nullable()
                    ->comment(
                        '視聴単価制の入札単価値は、a）小額の金額、b）AdWordsが自動的に選択した入札戦略で入札単価を設定している場合は「自動：x」または「自動」、'
                           . 'またはc）入札単価が適用されない場合は「 - 」のいずれかです行に'
                    );
                $table->double('ctr')
                    ->nullable()
                ...
                $table->double('convValueCurrentModel')
                    ->nullable()
                    ->comment(
                        '現在選択しているアトリビューションモデルで、過去の「ConversionValue」データがどのように表示されるかを示します。このフィールドは、'
                        . '小数点の区切り文字として小数点以下2桁の小数点以下桁区切り記号（例：1000000.00）のドット（"."）でフォーマットされます。'
                    );
                ...
                $table->bigInteger('interactions')
                    ->nullable()
                    ->comment('相互作用の数 インタラクションとは、テキストやショッピング広告のクリック、動画広告の表示など、広告フォーマットに関連する主要なユーザーアクションです。');
                $table->text('interactionTypes')
                    ->nullable()
                    ->comment('Interactions、InteractionRate、およびAverageCostの各列に反映される相互作用のタイプ。');
                ...
                $table->double('ptr')
                    ->nullable()
                    ->comment('受信した通話の数（NumOfflineInteractions）を、電話番号が表示された回数で割った数（NumOfflineImpressions）。');
                $table->date('quarter')
                    ->nullable()
                ...
                $table->string('targetCPASource', 50)
                    ->nullable()
                    ->comment('目標コンバージョン単価が設定されたレベル。 これは広告グループレベルでのみ適用されます。');
                ...
                $table->double('valueConvCurrentModel')
                    ->nullable()
                    ->comment(
                        '現在選択しているアトリビューションモデルで、過去の「ValuePerConversion」データがどのように表示されるかを示します。このフィールドは、このフィールドは、'

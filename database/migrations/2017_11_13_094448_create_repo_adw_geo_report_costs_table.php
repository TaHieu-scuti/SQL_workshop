        ...
        Schema::create(
            ...
            function (Blueprint $table) {
                ...
                $table->string('adType', 50)
                    ->nullable()
                    ->comment(
                        '広告の基礎となるメディア形式。 値は[テンプレート広告]ページ'
                        . '（URL：https://developers.google.com/adwords/api/docs/appendix/templateads）、'
                            . 'またはMediaType（URL：https://developers.google.com/adwords/api/docs/reference/latest/'
                        . 'AdGroupAdService.Media.MediaType）列挙型です。'
                    );
                ...
                $table->double('allConv')
                    ->nullable()
                    ->comment(
                        'AdWordsが推進するコンバージョン数の最善の見積もり。ウェブサイト、クロスデバイス、電話通話のコンバージョンが含まれます。このフィールドは、'
                ...
                $table->double('avgCPV')
                    ->nullable()
                    ->comment('ユーザーが広告を表示するたびに支払う平均金額。平均CPVは、すべての広告ビューの総コストをビュー数で割った値で定義されます。');
                ...
                $table->integer('city')
                    ->nullable()
                    ->comment(
                        '印象に関連付けられた都市のID。 LocationCriterionService（URL：https://developers.'
                        . 'google.com/adwords/api/docs/reference/latest/LocationCriterionService）を使用して、'
                        . '対応する名前やその他の情報を検索できます。'
                    );
                ...
                $table->integer('countryTerritory')
                    ->nullable()
                    ->comment(
                        'インプレッションに関連付けられた国のID。 LocationCriterionService ('
                        . 'URL：https://developers.google.com/adwords/api/docs/reference/'
                        . 'latest/LocationCriterionService)を使用すると、対応する名前やその他の情報を参照できます。'
                    );
                $table->double('crossDeviceConv')
                    ->nullable()
                    ->comment(
                        '顧客が1つの端末でAdWords広告をクリックしてから別の端末やブラウザで変換した後のコンバージョンデバイス間のコンバージョンは既にAll'
                        . 'Conversions列に含まれています。このフィールドは、小数点の区切り文字としてドット（"."）でフォーマットされます（例：1000000.00）。'
                    );
                $table->double('ctr')
                    ->nullable()
                ...
                $table->boolean('isTargetable')
                    ->nullable()
                    ->comment('行の場所（インプレッションに関連付けられたすべての場所の中）が、その行のインプレッションのターゲティングの場所であるかどうかを示します。');
                $table->string('locationType', 50)
                    ->nullable()
                    ->comment(
                        '場所のタイプ。 AREA_OF_INTERESTは、検索された場所、または表示されたコンテンツから派生した場所を示します。 '
                        . 'LOCATION_OF_PRESENCEは、ユーザーの実際の物理的な場所です。'
                    );
                $table->integer('metroArea')
                    ->nullable()
                    ->comment(
                        'メトロエリアのID印象に関連付けられた場所。 LocationCriterionService（URL：'
                        . 'https://developers.google.com/adwords/api/docs/reference/'
                        . 'latest/LocationCriterionService）を使用して、対応する名前やその他の情報を検索できます。'
                    );
                $table->string('month', 50)->nullable()->comment('月の最初の日。yyyy-MM-ddの形式です。');
                $table->string('monthOfYear', 50)->nullable()->comment('月の名前です（例：「12月」）。');
                $table->bigInteger('mostSpecificLocation')
                    ->nullable()
                    ->comment(
                        'インプレッションに関連付けられた最も具体的なロケーション基準のID。 LocationCriterionService'
                        . '（URL：https://developers.google.com/adwords/api/docs/reference'
                        . '/latest/LocationCriterionService）を使用して、対応する名前やその他の情報を検索できます。'
                    );
                $table->date('quarter')
                ...
                $table->integer('region')
                    ->nullable()
                    ->comment(
                        'インプレッションに関連付けられた地域のID。 LocationCriterionService'
                        . '（URL：https://developers.google.com/adwords/api/docs/'
                        . 'reference/latest/LocationCriterionService）を使用して、対応する名前やその他の情報を検索できます。'
                    );
                ...
                $table->bigInteger('viewThroughConv')
                    ->nullable()
                    ->comment(
                        'ビュースルーコンバージョンの合計数。これは、ディスプレイネットワーク広告が表示された後、後で他の広告とやり取り'
                           . '（クリックなど）せずにサイトのコンバージョンを達成した場合に発生します。このフィールドは、米国のロケールを使用してフォーマットされています'
                           . '。つまり、3桁区切り「,」、小数点区切りは「.」を使用しています。'
                    );

...
class CreateRepoAdwAdReportCostsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('repo_adw_ad_report_cost', function (Blueprint $table) {
            $table->increments('id');
            ...
            $table->string('timeZone', 50)->nullable()
                ->comment('顧客アカウント用に選択されたタイムゾーンの名前。 たとえば、「（GMT-05：00）東部時間」などです。
            ...
            $table->boolean('allowFlexibleColorResponsive')->nullable()->comment('応答性の高いディスプレイ広告の色を厳密に使用する必要があるかどうか。');
            ...
            $table->double('costConvCurrentModel')->nullable()->comment('現在選択しているアトリビューションモデルで、過去の「CostPerConversion」データがどのように表示されるかを示します。このフィールドは、小数点の区切り文字としてドット（"."）でフォーマットされます（例：1000000.00）。');
            $table->text('destinationURL')->nullable()->comment('広告のリンク先URL。');
            $table->text('appFinalURL')->nullable()->comment('広告の最終的なアプリURLのリスト。 リストのエントリは、a）「android-app：」（Androidアプリの場合）またはb）「os-app：」（iOSアプリの場合）のいずれかで始まります。 リスト要素はJSONリスト形式で返されます。');
            ...
            $table->text('customParameter')->nullable()->comment('広告のカスタムパラメータのリスト。 CustomParameters要素はJSONマップ形式で返されます。');
            ...
            $table->double('conversionsCurrentModel')->nullable()->comment('現在選択しているアトリビューションモデルでの過去の「コンバージョン」データの表示方法を示します。このフィールドは、小数点の区切り文字としてドット（"."）でフォーマットされます（例：1000000.00）。');
            $table->double('convValueCurrentModel')->nullable()->comment('現在選択しているアトリビューションモデルで、過去の「ConversionValue」データがどのように表示されるかを示します。このフィールドは、小数点の区切り文字としてドット（"."）でフォーマットされます（例：1000000.00）。');
            ...
            $table->bigInteger('devicePreference')->nullable()->comment('デバイスプリファレンスのプラットフォームID。 Platformsリファレンスで、対応する名前やその他の情報を参照できます。 URL：https://developers.google.com/adwords/api/docs/appendix/platforms');
            ...
            $table->bigInteger('imageIDResponsive')->nullable()->comment('ResponsiveDisplayAdで使用されるマーケティングイメージのID。');
            ...
            $table->text('ad')->nullable()->comment('extAdの広告見出し。 TemplateAdなどの他の広告タイプの場合、このフィールドには広告のキー属性の文字列表現が含まれます。');
            ...
            $table->text('imageAdURL')->nullable()->comment('完全なURLを取得するには、この値の前に「https://tpc.googlesyndication.com/pageadimg/imgad?id=」と入力します。');
            ...
            $table->string('adState', 50)->nullable()->comment('この行のメインオブジェクトのステータス。たとえば、キャンペーンの掲載結果レポートでは、これが各行のキャンペーンのステータスになります。広告グループの掲載結果レポートでは、これは各行の広告グループのステータスになります。');
            ...
            $table->double('valueConvCurrentModel')->nullable()->comment('現在選択しているアトリビューションモデルで、過去の「ValuePerConversion」データがどのように表示されるかを示します。このフィールドは、小数点の区切り文字としてドット（"."）でフォーマットされます（例：1000000.00）。');

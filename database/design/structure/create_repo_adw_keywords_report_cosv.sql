 ...
 * Table Name : repo_adw_keywords_report_conv
...
CREATE TABLE IF NOT EXISTS `ADGAINER_db_SECURE`.`repo_adw_keywords_report_conv` (
	...
	`firstPageCPC` Double NULL COMMENT '検索結果の最初のページに広告を表示するために必要なクリック単価の見積もり。通常マイクロ秒単位の数字ですが、自動入札機能が使用されている場合は「auto：」という接頭辞が付いていてもよく、単に「auto」という文字列であってもかまいません。',
	`firstPositionCPC` Double NULL COMMENT '広告がGoogle検索結果の最初のページの最初の位置に表示されるのに必要な金額を見積もります。通常マイクロ秒単位の数字ですが、自動入札機能が使用されている場合は「auto：」という接頭辞が付いていてもよく、単に「auto」という文字列であってもかまいません。',
	...
	`topOfPageCPC` Double NULL COMMENT '検索結果の最初のページの上部に広告を表示するために必要なクリック単価の見積もり。通常マイクロ秒単位の数字ですが、自動入札機能が使用されている場合は「auto：」という接頭辞が付いていてもよく、単に「auto」という文字列であってもかまいません。',

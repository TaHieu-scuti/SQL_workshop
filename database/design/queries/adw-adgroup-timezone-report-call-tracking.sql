SELECT
	...
	`repo_adw_adgroup_report_cost`.`hourOfDay` AS timezone,
...
FROM
	`repo_adw_adgroup_report_cost`
		...
		ON (
			...
			AND
				HOUR(`phone_time_use`.`time_of_call`) = `repo_adw_adgroup_report_cost`.`hourOfDay`
		)
WHERE
...
GROUP BY
	...
	`repo_adw_adgroup_report_cost`.`hourOfDay`

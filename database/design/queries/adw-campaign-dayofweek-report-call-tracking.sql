SELECT
	...
	`repo_adw_campaign_report_cost`.`dayOfWeek` AS dayOfWeek,
...
FROM
	`repo_adw_campaign_report_cost`
		...
		ON (
			...
			AND
				DAYNAME(`phone_time_use`.`time_of_call`) = `repo_adw_campaign_report_cost`.`dayOfWeek`
		)
WHERE
...
GROUP BY
	...
	`repo_adw_campaign_report_cost`.`dayOfWeek`

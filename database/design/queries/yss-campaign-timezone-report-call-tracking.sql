SELECT
	...
	`repo_yss_campaign_report_cost`.`hourofday` AS timezone,
...
FROM
	`repo_yss_campaign_report_cost`
		...
		ON (
			...
			AND
				HOUR(`phone_time_use`.`time_of_call`) = `repo_yss_campaign_report_cost`.`hourofday`
		)
WHERE
...
GROUP BY
	...
	`repo_yss_campaign_report_cost`.`campaign_id`,
	`repo_yss_campaign_report_cost`.`accountid`,
	`repo_yss_campaign_report_cost`.`hourofday`

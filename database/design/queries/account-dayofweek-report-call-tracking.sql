...
FROM
  (
    SELECT
      ...
      `repo_adw_account_report_cost`.`dayOfWeek` AS dayOfWeek,
    ...
    GROUP BY
      `repo_adw_account_report_cost`.`account_id`,
      `repo_adw_account_report_cost`.`campaign_id`,
      `repo_adw_account_report_cost`.`dayOfWeek`
  ) AS adw,
  (
    SELECT
  ...
  (
    SELECT
      ...
      `repo_yss_account_report_cost`.`dayOfWeek` AS dayOfWeek,
    ...
    GROUP BY
      `repo_yss_account_report_cost`.`account_id`,
      `repo_yss_account_report_cost`.`campaign_id`,
      `repo_yss_account_report_cost`.`dayOfWeek`
  ) AS yss
WHERE
  `adw`.`dayOfWeek` = `ydn`.`dayOfWeek`
AND
  `ydn`.`dayOfWeek` = `yss`.`dayOfWeek`

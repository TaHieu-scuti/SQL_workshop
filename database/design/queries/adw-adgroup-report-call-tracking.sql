...
FROM
  (
    ...
    WHERE
      ...
      AND
      `repo_adw_ad_report_cost`.`customerID` = 11111
      AND
      `repo_adw_ad_report_cost`.`campaignID` = 111111
      AND
    ...
    FROM
      `repo_adw_adgroup_report_cost`
        ...
        ON (
          ...
          AND
            STR_TO_DATE(`phone_time_use`.`time_of_call`, '%Y-%m-%d') = `repo_adw_adgroup_report_cost`.`day`
          AND
            `phone_time_use`.`source` = 'adw'
          AND
            `phone_time_use`.`traffic_type` = 'AD'
    ...
    AND
      `repo_adw_adgroup_report_cost`.`customerID` = 11111
    AND
      `repo_adw_adgroup_report_cost`.`campaignID` = 111111
    AND

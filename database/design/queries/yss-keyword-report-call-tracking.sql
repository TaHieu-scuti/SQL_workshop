...
SELECT
  ...
  SUM(`repo_yss_keyword_report_cost`.`conversions`) AS web_cv,
...
FROM
  `repo_yss_keyword_report_cost`
    ...
    ON (
      ...
      AND
        `phone_time_use`.`account_id` = `repo_yss_keyword_report_cost`.`account_id`
      AND
        `phone_time_use`.`campaign_id` = `repo_yss_keyword_report_cost`.`campaign_id`
      AND
        `phone_time_use`.`utm_campaign` = `repo_yss_keyword_report_cost`.`campaignID`
      AND
        STR_TO_DATE(`phone_time_use`.`time_of_call`, '%Y-%m-%d') = `repo_yss_keyword_report_cost`.`day`
      AND
...
AND
  `repo_yss_keyword_report_cost`.`accountid` = 11111
AND
  `repo_yss_keyword_report_cost`.`campaignID` = 111111
AND
  `repo_yss_keyword_report_cost`.`adgroupID` = 111111111111110
AND

...
FROM
  `repo_adw_keywords_report_cost`
    ...
    ON (
      ...
      AND
        `phone_time_use`.`account_id` = `repo_adw_keywords_report_cost`.`account_id`
      AND
        `phone_time_use`.`campaign_id` = `repo_adw_keywords_report_cost`.`campaign_id`
      AND
        `phone_time_use`.`utm_campaign` = `repo_adw_keywords_report_cost`.`campaignID`
      AND
        STR_TO_DATE(`phone_time_use`.`time_of_call`, '%Y-%m-%d') = `repo_adw_keywords_report_cost`.`day`
      AND

...
JOIN
  `repo_phone_time_use`
  ON
      `repo_phone_time_use`.`account_id` = `repo_adw_adgroup_report_cost`.`account_id`
    AND
      `repo_phone_time_use`.`campaign_id` = `repo_adw_adgroup_report_cost`.`campaign_id`
    AND
      `repo_phone_time_use`.`utm_campaign` = `repo_adw_adgroup_report_cost`.`campaignID`
    AND
      `repo_adw_adgroup_report_cost`.`day` = STR_TO_DATE(`repo_phone_time_use`.`time_of_call`, '%Y-%m-%d')
    AND
...
AND
  `repo_adw_adgroup_report_cost`.`device` = 'UNKNOWN'
AND
...
AND
  ...
  OR
    `repo_adw_adgroup_report_cost`.`network` = 'CONTENT'
  )
GROUP BY
  device
...
AND
  `repo_adw_adgroup_report_cost`.`campaignID` = 11
AND
  `repo_adw_adgroup_report_cost`.`device` = 'DESKTOP'
AND
  `repo_adw_adgroup_report_cost`.`day` >= '2017-01-01'
AND
  `repo_adw_adgroup_report_cost`.`day` <= '2017-12-01'
...
AND
  `repo_adw_adgroup_report_cost`.`campaignID` = 11
AND
  `repo_adw_adgroup_report_cost`.`device` = 'HIGH_END_MOBILE'
AND
  `repo_adw_adgroup_report_cost`.`day` >= '2017-01-01'
AND
  `repo_adw_adgroup_report_cost`.`day` <= '2017-12-01'

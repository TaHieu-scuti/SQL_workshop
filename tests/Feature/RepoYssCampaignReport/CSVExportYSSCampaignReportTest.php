...
class CSVExportYSSCampaignReportTest extends TestCase
{
    ...
    const CUSTOM_START_DAY = '2017-07-08';
    const CUSTOM_END_DAY = '2017-10-06';
    ...
    public function testReturnsStatus200()
    {
        ...
        $response = $this->actingAs($user)
                        ...
                        ->get('/campaign-report/export_csv');
    ...
    public function testReturnsCorrectResponseHeaders(array $response)
    {
        ...
        $fileName = $response['now']->format("Y_m_d h_i ") . 'repo_yss_campaign_report_costs.csv';
    ...
    {
        $expectedContent = file_get_contents(__DIR__ . '/../../resources/repo_yss_campaign_report_costs.csv');
        $this->assertSame($expectedContent, $response['response']->getContent());
    }

    public function testRedirectsToLoginRouteWhenNotLoggedIn()
    {
        $response = $this->get('/campaign-report/export_csv');
